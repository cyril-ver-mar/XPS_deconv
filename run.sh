...
EOF
  ...
  echo
  printf '%s  If the browser does not open automatically:%s\n' "$C_BOLD" "$C_RESET"
  printf '  %s·%s open %shttp://localhost:8501%s yourself\n' "$C_ACCENT" "$C_RESET" "$C_BOLD" "$C_RESET"
  printf '  %s·%s or try %shttp://127.0.0.1:8501%s\n' "$C_ACCENT" "$C_RESET" "$C_BOLD" "$C_RESET"
  printf '  %s·%s if the port is busy, stop the other Streamlit and re-run\n' "$C_ACCENT" "$C_RESET"
  echo
}
