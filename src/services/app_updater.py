...
# Never overwrite user runtime / env / VCS when applying an update zip.
...
def apply_standalone_zip(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="xps_deconv_upd_") as tmp:
        tmp_path = Path(tmp)
...
def download_and_apply(zip_url: str, *, app_root: Optional[Path] = None) -> Path:
    ...
    target = (app_root or ROOT).resolve()
    with tempfile.TemporaryDirectory(prefix="xps_deconv_dl_") as tmp:
