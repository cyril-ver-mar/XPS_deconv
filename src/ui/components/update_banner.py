...
from src.utils.github_updates import UpdateStatus, check_for_update
...
def render_update_banner(lang: str) -> None:
    """Show update info in the main area when a newer GitHub release exists."""
    ...
    if latest.zip_url:
        with st.expander(t("update_install_expander", lang), expanded=False):
        ...
        st.caption(t("update_no_zip", lang))

    if st.button(t("update_dismiss", lang), key="_update_dismiss_btn"):
...
def render_update_settings(lang: str) -> None:
    ...
    if not status.configured:
        st.warning(t("update_not_configured", lang))
    else:
