...
with left:
    with st.expander(t("denoise", lang), expanded=False):
        labeled_help(t("denoise", lang), "denoise_method", lang)
        noise_method = st.selectbox(t("method", lang), list(DENOISE_METHODS), key="ws_noise_method")
    ...
    with st.expander(t("baseline", lang), expanded=False):
        labeled_help(t("baseline", lang), "baseline_method", lang)
        method = st.selectbox(
            t("baseline", lang),
