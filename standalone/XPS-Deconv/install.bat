...
echo   ^|                                              ^|
echo   +----------------------------------------------+%C_RESET%
echo %C_DIM%  Thermo VGD -^> baseline -^> deconvolution%C_RESET%
echo.
...
if errorlevel 1 (
  ...
  echo.
  echo %C_BOLD%How to fix%C_RESET%
  echo   %C_ACCENT%·%C_RESET% Reinstall Python 3.11 with "pip" / standard library
  echo   %C_ACCENT%·%C_RESET% Then re-run: install.bat
  echo.
  exit /b 1
)
...
if not exist requirements.txt (
  ...
  echo.
  echo %C_BOLD%How to fix%C_RESET%
  echo   %C_ACCENT%·%C_RESET% Run install.bat from the XPS-Deconv project root
  echo.
  exit /b 1
)
...
echo %C_DIM%  Docs inside the app: Documentation page%C_RESET%
echo.
