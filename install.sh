#!/usr/bin/env bash
# XPS-Deconv installer — clean Claude-style terminal UX
set -euo pipefail

ROOT="$(cd "$(dirname "$0")" && pwd)"
cd "$ROOT"

# --- colors (Claude-like coral accent) ---------------------------------
...
banner() {
  ...
  ╰──────────────────────────────────────────────╯
EOF
  printf '%s\n' "${C_RESET}${C_DIM}  Thermo VGD → baseline → deconvolution${C_RESET}"
  echo
}
...
if [[ ! -f requirements.txt ]]; then
  fail "requirements.txt not found" \
    "Run this script from the XPS-Deconv project root" \
    "Expected file: $ROOT/requirements.txt"
fi
if ! "$VPY" -m pip install --upgrade pip >/tmp/xps_deconv_pip_up.log 2>&1; then
  fail "Failed to upgrade pip" \
    "See /tmp/xps_deconv_pip_up.log" \
...
if ! "$VPIP" install -r requirements.txt; then
  fail "pip install -r requirements.txt failed" \
    ...
    "On corporate networks you may need a proxy or mirror"
fi
...
echo
tip "./run.sh"
printf '%s  Docs inside the app: Documentation page · files in docs/%s\n' "$C_DIM" "$C_RESET"
echo
